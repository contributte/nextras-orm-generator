...
use Minetro\Resolver\IRepositoryResolver;
...
class RepositoryGenerator extends AbstractGenerator
{
    ...
    public function generate(Database $database)
    {
        foreach ($database->getTables() as $table) {
            ...
            // Save file
            $this->generateFile($this->config->get('repository.folder'), $this->resolver->resolveRepositoryFilename($table), (string)$namespace);
        }
        ...
        if ($this->config->get('repository.extends') !== NULL) {
            ...
            $extends = $this->config->get('orm.class.repository');
            $namespace->addUse($extends);
            $class->setExtends($extends);

            // Save file
            $this->generateFile($this->config->get('repository.folder'), $this->resolver->resolveFilename(Helpers::extractShortName($this->config->get('repository.extends'))), (string)$namespace);
        }
    }
